...
from torchtrade.env.generic import RewardScheme, TradingEnv
from torchtrade.feed.core import Stream, DataFeed


class TorchTradeRewardScheme(RewardScheme):
    ...
    def get_reward(self, portfolio) -> float:
        ...
        ----------
        portfolio : `Portfolio`
            The portfolio associated with the `TorchTradeActionScheme`.
...
class SimpleProfit(TorchTradeRewardScheme):
...
class RiskAdjustedReturns(TorchTradeRewardScheme):
...
class PBR(TorchTradeRewardScheme):
...
def get(identifier: str) -> 'TorchTradeRewardScheme':
