...
def _create_wallet_source(wallet: 'Wallet', include_worth: bool = True) -> 'List[Stream[float]]':
    ...
    with NameSpace(exchange_name + ":/" + symbol):
        ...
        if include_worth:
            ...
            worth = price.current * total_balance.obj.balance
            worth_sensor = Stream.sensor(worth, lambda node: node).rename('worth')
            # worth = price.current.mul(total_balance).rename('worth')
            streams += [worth_sensor]
...
class TorchTradeObserver(Observer):
    """The TorchTrade observer that is compatible with the other `default`
    components.
